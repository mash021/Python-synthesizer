# oberheim_sem_filter.py (Oberheim SEM Filter Implementation)
import numpy as np

class OberheimSEMFilter:
    ...
    def process(self, input_signal):
        ...
        for i in range(len(input_signal)):
            x = input_signal[i] - self.resonance_tuned * self.buffer[3]
            ...
            self.buffer[2] += self.cutoff_tuned * (np.tanh(self.buffer[1]) - np.tanh(self.buffer[2]))
            self.buffer[3] += self.cutoff_tuned * (np.tanh(self.buffer[2]) - np.tanh(self.buffer[3]))
            output[i] = self.buffer[3]

        return output

# Example usage:
# filter = OberheimSEMFilter(sample_rate=96000, cutoff=200, resonance=0.7)
